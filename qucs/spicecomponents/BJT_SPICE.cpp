...
BJT_SPICE::BJT_SPICE()
{
    ...
    Props.append(new Property("Letter", "Q", true,"[Q, X] BJT type"));
...
QString BJT_SPICE::spice_netlist(bool)
{
 ...
 //   if(  Q.length()  > 0)          s += QString("%1").arg(Q);
    if(  Q_Line_2.length() > 0 )   s += QString("%1").arg(Q_Line_2);
    if(  Q_Line_3.length() > 0 )   s += QString("%1").arg(Q_Line_3);
    if(  Q_Line_4.length() > 0 )   s += QString("%1").arg(Q_Line_4);
    if(  Q_Line_5.length() > 0 )   s += QString("%1").arg(Q_Line_5);
    s += "\n";
